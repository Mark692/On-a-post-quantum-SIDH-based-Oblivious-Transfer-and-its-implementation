...
{// Build an entangled basis for E[2^m].
 ...
 //Definizioni dei tipi di variabili presenti:
    //felm_t    = Datatype for representing 503-bit field elements (512-bit max.)
    //f2elm_t   = Datatype for representing quadratic extension field elements GF(p503^2). Consiste in una coppia di valori (Reale, Complesso)
    //point_t   = Point representation in affine coordinates. Consiste in una coppia (x, y)

 //Inputs alla funzione:
 //                      f2elm_t A  =  coefficiente della curva espressa in forma di Montgomery, proiettiva
 //Outputs della funzione:
//                      point_t S1 = primo   punto della base 2^n. Viene espresso in coordinate affini (x, y)
//                      point_t S2 = secondo punto della base 2^n. Viene espresso in coordinate affini (x, y)
//                      unsigned char *bit = è ausiliario alla funzione di decompressione. Corrisponde a dire se A sia un quadrato oppure no
//                      unsigned char *entry = è ausiliario alla funzione di decompressione. Corrisponde all'indice in cui si trova il parametro "t" nella tabella table_v_qnr o table_v_qr
    ...
    fpcopy((digit_t*)&Montgomery_one, one[0]); //Questa funzione semplicemente copia il valore Montgomery_one all'interno di one[0]
    //Montgomery_one = { 0x00000000000003F9, 0x0000000000000000, 0x0000000000000000, 0xB400000000000000, 0x63CB1A6EA6DED2B4, 0x51689D8D667EB37D, 0x8ACD77C71AB24142, 0x0026FBAEC60F5953 };
    //Nota importante: è molto probabile che il numero sopra riportato sia in notazione Big Endian e quindi andrebbe letto a partire dagli ultimi 16bit a destra

    copy_words((const digit_t *)u0_entang, u0[0], 2*NWORDS_FIELD); //Analogo a sopra, viene copiato il valore di u0_entang all'interno di u0[0]
    copy_words((const digit_t *)u_entang, u[0], 2*NWORDS_FIELD);
    //NWORDS_FIELD = 8 oppure 16, varia in base al processore utilizzato (16 se a 64bit)
    ...
    *bit = isSqrA;  // This bit is transmitted along with the PK to speedup decompression
    ...
    do {
        ...
        index += 2;
    } while (!is_sqr_fp2(t, s)); //Original code
