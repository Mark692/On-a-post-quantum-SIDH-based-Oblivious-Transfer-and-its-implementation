...
{ // Bob's ephemeral public key generation
  ...
  // Output: the public key PublicKeyB consisting of 3 elements in GF(p^2) which are encoded by removing leading 0 bytes.
    ...
    // Retrieve kernel point
    LADDER3PT(XPB, XQB, XRB, (digit_t*)PrivateKeyB, BOB, R, A); //Returns R=P+Q dove R è il kernel per la successiva isogenia
    
    // Traverse tree
    //Calcola phi(E):E->E', dove E' ~= E/<R>
    index = 0;
    ...
    fp2mul_mont(phiQ->X, phiQ->Z, phiQ->X);
    fp2mul_mont(phiR->X, phiR->Z, phiR->X);
    //Fine albero di isogenie

    // Format public key
    fp2_encode(phiP->X, ct_uncomp); //Scrive sulla stringa ct_uncomp la x di phi(P)
...
{ // Alice's ephemeral shared secret computation
  ...
  // Output: a shared secret SharedSecretA that consists of one element in GF(p^2). 
    ...
    fp2copy(param_A, A); //A = param_A preso dalla chiave compressa di Alice
    
    fpadd((digit_t*)&Montgomery_one, (digit_t*)&Montgomery_one, C24[0]);//C24 = 2
    fp2add(A, C24, A24plus); //A24plus = A + 2
    fpadd(C24[0], C24[0], C24[0]); //C24 = 4
...
{ // Bob's ephemeral shared secret computation
  ...
  // Output: a shared secret SharedSecretB that consists of one element in GF(p^2). 
    ...
    fpadd((digit_t*)&Montgomery_one, (digit_t*)&Montgomery_one, A24minus[0]); //A24m = 2
    fp2add(A, A24minus, A24plus); //A24p = 8
    fp2sub(A, A24minus, A24minus); //A24m = 4
    ...
    get_3_isog(R, A24minus, A24plus, coeff); //Fine traverse tree
    fp2add(A24plus, A24minus, A); //A=8+4=12
    fp2add(A, A, A); //A = 24
    fp2sub(A24plus, A24minus, A24plus); //A24p = 4
